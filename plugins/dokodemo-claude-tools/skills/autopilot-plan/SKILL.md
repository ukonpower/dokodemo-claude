...
- （評価リクエスト運用があるプロジェクトのみ）docs/direction.md と、docs/feedback-log.md の未蒸留件数（`grep -c "\[未蒸留\]" docs/feedback-log.md`）。tasks.md の REVIEW（評価待ち）レーンの滞留も見る
...
4. コミットは autopilot.md の手順6の運用に合わせる。AI 自身がコミットする運用なら `chore({{スコープ}}): 計画ターン` 等としてコミットする（記録ファイルのみの変更であることを git status で確認してから）。コミットをループ実行基盤に任せる運用なら `git commit` は実行せず、記録ファイルの変更を作業ツリーに残して停止する
5. 終了報告には以下を1行ずつ含めて停止する:
   - 到達ラインへの現在地の評価
   - tasks.md に加えた変更（なければ「変更なし」と理由）
   - 次の作業ターンが最初に着手すべきタスク
   - （評価リクエスト運用のみ）feedback-log の未蒸留が5件以上なら「蒸留ターン（/dokodemo-claude-tools:autopilot-distill）の実行を推奨」。**このターンで蒸留は実行しない**（蒸留はユーザーの明示指示で行う手動運用）
