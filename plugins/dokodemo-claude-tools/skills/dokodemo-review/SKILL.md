...
発行するのは**ユーザーの判断が成果物を実質的に変えるもの**だけ:

- **まとまった成果の評価**: 1つのゴール・マイルストーン分の変更をまとめて1件で問う
- **複数案からの選択**: どれを採用するかユーザーに選んでほしいとき
- **方向性の分岐**: このまま進むと大きな手戻りになりうる選択

**細部の判断（余白・位置・サイズ・整列など）は発行しない。** プロジェクトの
方向性文書（direction 等）や過去に承認された前例に従って自分で判断する。
細かい問いを毎回発行すると、受信箱が「OKを押すだけの追認」で埋まり、
本当に判断が要るレビューが埋もれる。内部ロジックのみの変更でも発行しない。
