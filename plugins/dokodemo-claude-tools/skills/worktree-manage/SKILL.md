...
- **「これらのタスクをそれぞれワークツリーを立てて実装して」「ワークツリーを切って対応して」** のように
  ...
  （「ブランチ命名はリポジトリの規約に合わせる」を参照）。**作成時は必ず以下を明示的に設定する**:
  - `baseBranch`（分岐元ブランチ）— 省略すると現在の HEAD から切ってしまう事故が起きる
  - `description`（タスク諸情報）— タイトル・Base・やること等を下記テンプレに沿って Markdown で必ず書く。
    issue/PR の URL が渡されていればリンクとして含める
...
3. **分岐元ブランチ（`baseBranch`）を決める。** 原則そのリポジトリの既定ブランチ
   （`main` / `master` / `develop` など、リポジトリの慣習に従う）から切る。ユーザーが明示的に
   分岐元を指定していればそれに従う。`release/*` や `hotfix/*` のようにベースが自明でないケース
   （進行中のリリースブランチに合流させたい、等）だけ 1 度確認する。省略はしない
   （省略すると現在の HEAD から切って意図せず別作業ブランチから派生する事故が起きる）。
4. **各タスクを `worktree_create` でワークツリー化**（`baseBranch` を明示的に渡す。
   `description` はテンプレを埋めた Markdown で必ず記載。レスポンスの `worktree.wtid` を控える）。
5. **タスクを委譲する**。実装・修正・調査は `prompt_broadcast` で対象 wtid の AI キューへ
   （`includeMain: false`）。`npm run dev` 等の明示コマンドは `terminal-control` でそのワークツリーの
   terminal へ（下記「AI 委譲か terminal 実行かの振り分け」）。**作成側はコードを書かない。**
6. 作成したワークツリー（ブランチ / **分岐元** / wtid / 委譲内容）を簡潔に報告して終わる。
...
下記テンプレを埋める形で書けば必要項目が漏れない。

**必須項目**（必ず埋める）:

- **タイトル** — そのワークツリーで何をするかを一行で要約（見出しや太字で簡潔に）
- **`Base:`** — 分岐元ブランチ名。「後からどのブランチから切ったか」を追跡できるよう必ず書く
  （値の決め方は下記「`Base:` に書く値のルール」を参照）
- **やること** — 着手内容を箇条書きで数点

**推奨項目**（ユーザーから渡されていれば必須）:

- **Issue URL** — `#123` のような番号だけでなく開けるリンクにする
- **PR URL** — 関連 PR がある場合
- **受け入れ条件（完了条件）** — 何ができたら完了か
- **関連ドキュメント URL** — 仕様・要求・参考資料
- **メモ** — 制約・前提・注意点など
...
- 受け入れ条件: 不正なメール形式で送信時にエラーメッセージが表示され、正しい形式なら送信できる

やること:
- メール形式チェック（RFC 準拠の正規表現）を追加
- フォーム下にエラーメッセージ表示領域を追加
- 既存のユニットテストを更新
```

##### `Base:` に書く値のルール

- **`baseBranch` を渡した場合** → その値をそのまま書く（例: `main`, `develop`, `release/v0.2.0`）
- **`baseBranch` を渡さなかった場合** → 作成時点の HEAD ブランチ名
  （`git rev-parse --abbrev-ref HEAD` の結果）を書く
- **デタッチド HEAD の場合** → commit ハッシュの先頭 7 文字を書く（例: `1facddc`）
...
| `baseBranch` | 原則明示 | 新規ブランチの分岐元。**省略しない**（省略すると現在の HEAD から切ってしまい、意図せず別作業ブランチから派生する事故が起きる）。原則そのリポジトリの既定ブランチ（`main` / `develop` 等）を渡す。ここに渡した値を **`description` の `Base:` にも同じ値で書く**こと |
...
- **`baseBranch` は省略しない。** 省略すると現在の HEAD から切ってしまい、意図せず別作業ブランチから
  派生する事故になる。原則そのリポジトリの既定ブランチ（`main` / `develop` 等）を明示し、
  同じ値を `description` の `Base:` にも書く。
- **description には諸情報を全部盛りする。** タイトル・`Base:`・やること（必須）、
  issue / PR / 受け入れ条件 / 参考資料（渡されていれば必須）を「タブを見ただけで思い出せる」
  レベルまで埋める。書き漏らしはテンプレを埋める形で防ぐ。
