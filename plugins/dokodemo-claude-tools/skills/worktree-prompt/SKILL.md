...
| `model` | 任意 | 使用モデルの指定。**省略時は呼び出し元セッション（Step 1 で rid を取得したパス）で最後に適用されたモデルを自動継承する**（呼び出し元にモデル指定の履歴が無ければ未指定のまま） |
...
`model` / `modelInherited` は実際に各ワークツリーへ適用されたモデルと、それが `model`
未指定による自動継承の結果かどうかを示す。明示的に `model` を指定した場合は
`modelInherited: false` になる。
