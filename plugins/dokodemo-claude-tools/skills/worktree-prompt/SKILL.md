...
curl -sk -X POST "https://localhost:${API_PORT}/api/prompt/broadcast" \
  -H "Content-Type: application/json" \
  -d "{\"rid\":\"${RID}\",\"provider\":\"claude\",\"prompt\":\"テストを実行して\"}" | jq
```

## ワークフロー

### Step 1: API のポートと rid を取得する
...
```bash
curl -sk "https://localhost:${API_PORT}/api/worktrees/${RID}" | jq '.worktrees'
```
...
curl -sk -X POST "https://localhost:${API_PORT}/api/prompt/broadcast" \
  -H "Content-Type: application/json" \
  -d "{\"rid\":\"${RID}\",\"provider\":\"claude\",\"prompt\":\"npm run lint を通して\"}" | jq

# 対象を絞って送信（main も含める）
curl -sk -X POST "https://localhost:${API_PORT}/api/prompt/broadcast" \
  -H "Content-Type: application/json" \
  -d "{\"rid\":\"${RID}\",\"provider\":\"claude\",\"prompt\":\"...\",\"targets\":[\"wtid1\",\"wtid2\"],\"includeMain\":true}" | jq
```
...
- 日本語プロンプトを `-d` でインライン指定するときは JSON 文字列のエスケープに注意。複雑なら一時ファイル + `--data @file` を使う。
