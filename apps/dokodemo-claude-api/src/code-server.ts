...
export async function startCodeServer(): Promise<CodeServer> {
  ...
  try {
    ...
    const useHttps = process.env.DC_USE_HTTPS !== 'false';
    const protocol = useHttps ? 'https' : 'http';
    const url = `${protocol}://localhost:${availablePort}`;
    ...
    const codeServerArgs = [
      '--auth',
      'none',
      '--disable-telemetry',
      '--bind-addr',
      `0.0.0.0:${availablePort}`,
    ];
    if (useHttps) {
      const certPath = process.env.DC_HTTPS_CERT_PATH;
      const keyPath = process.env.DC_HTTPS_KEY_PATH;
      if (certPath && keyPath) {
        codeServerArgs.push('--cert', certPath);
        codeServerArgs.push('--cert-key', keyPath);
      } else {
        console.warn(
          '[code-server] HTTPS有効ですが DC_HTTPS_CERT_PATH / DC_HTTPS_KEY_PATH が未設定のため、--cert フラグを付けずに起動します。'
        );
      }
    }

    codeServerProcess = spawn('code-server', codeServerArgs, {
      stdio: ['ignore', 'pipe', 'pipe'],
      env: cleanChildEnv({ VSCODE_IPC_HOOK_CLI: undefined }),
    });

    codeServerInstance.pid = codeServerProcess.pid;

    // 標準出力をログ（起動完了検出のみ、通常出力は抑止）
    codeServerProcess.stdout?.on('data', (data) => {
      const output = data.toString();

      // 起動完了を検出（HTTPSもHTTPも対応）
    ...
    if (
      ...
      error.message.includes('No available port found')
    ) {
...
export async function stopCodeServer(): Promise<void> {
  ...
  if (codeServerProcess && codeServerInstance.pid) {
    try {
      ...
      console.error(
        `Failed to stop code-server (PID: ${codeServerInstance.pid}):`,
        error
      );
      throw error;
    }
